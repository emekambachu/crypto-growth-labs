...
$('.users-list-box .person').on('click', function(event) {
    if ($(this).hasClass('.active')) {
        return false;
    } else {
        var findChat = $(this).attr('data-chat');
        var personName = $(this).find('.users-name').text();
        ...
        if (window.innerWidth <= 767) {
          $('.chat-box .current-chat-users-name .name').html(personName.split(' ')[0]);
        } else if (window.innerWidth > 767) {
          $('.chat-box .current-chat-users-name .name').html(personName);
        }
        $('.chat-box .current-chat-users-name img').attr('src', personImage);
        $('.chat').removeClass('active-chat');
        $('.users-list-box .person').removeClass('active');
        $('.chat-box .chat-box-inner').css('height', '100%');
        $(this).addClass('active');
        $('.chat[data-chat = '+findChat+']').addClass('active-chat');
    }
    if ($(this).parents('.users-list-box').hasClass('user-list-box-show')) {
      $(this).parents('.users-list-box').removeClass('user-list-box-show');
    }
    $('.chat-meta-users').addClass('chat-active');
...
$('.phone-call-screen').off('click').on('click', function(event) {
  var getCallingUserName = $(this).parents('.chat-system').find('.person.active .users-name').attr('data-name');
  var getCallingUserImg = $(this).parents('.chat-system').find('.person.active .f-head img').attr('src');
  var setCallingUserName = $(this).parents('.chat-box').find('.overlay-phone-call .users-name').text(getCallingUserName);
  var setCallingUserName = $(this).parents('.chat-box').find('.overlay-phone-call .calling-users-img img').attr('src', getCallingUserImg);
...
$('.switch-to-video-call').off('click').on('click', function(event) {
    var getCallerId = $(this).parents('.overlay-phone-call').find('.users-name').text();
    var getCallerImg = $(this).parents('.overlay-phone-call').find('.calling-users-img img').attr('src');
    $(this).parents('.overlay-phone-call').removeClass('phone-call-show');
    $('.overlay-video-call').addClass('video-call-show');
    $('.overlay-video-call').find('.users-name').text(getCallerId);
    $('.overlay-video-call').find('.calling-users-img img').attr('src', getCallerImg);
...
$('.cancel-call').on('click', function(event) {
    ...
    if ($(this).parents('.overlay-phone-call').hasClass('phone-call-show')) {
      ...
      var displayCallerImage = $(this).parents('.overlay-video-call').find('.calling-users-img').css('display', 'block');
...
function videoCallOnConnect() {
  ...
  var getCallStatusImage = $('.overlay-video-call .calling-users-img');
...
$('.video-call-screen').off('click').on('click', function(event) {
  var getCallingUserName = $(this).parents('.chat-system').find('.person.active .users-name').attr('data-name');
  var getCallingUserImg = $(this).parents('.chat-system').find('.person.active .f-head img').attr('src');
  var setCallingUserName = $(this).parents('.chat-box').find('.overlay-video-call .users-name').text(getCallingUserName);
  var setCallingUserName = $(this).parents('.chat-box').find('.overlay-video-call .calling-users-img img').attr('src', getCallingUserImg);
...
$('.switch-to-phone-call').off('click').on('click', function(event) {
    var getCallerId = $(this).parents('.overlay-video-call').find('.users-name').text();
    var getCallerImg = $(this).parents('.overlay-video-call').find('.calling-users-img img').attr('src');

    $(this).parents('.overlay-video-call').removeClass('video-call-show');
    $('.overlay-phone-call').addClass('phone-call-show');
    $('.overlay-phone-call').find('.users-name').text(getCallerId);
    $('.overlay-phone-call').find('.calling-users-img img').attr('src', getCallerImg);
...
$('.hamburger, .chat-system .chat-box .chat-not-selected p').on('click', function(event) {
  $(this).parents('.chat-system').find('.users-list-box').toggleClass('user-list-box-show')
})
